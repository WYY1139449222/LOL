        ...
        selectedDay: function (d, type) {
            var arr = this.getDisDateValue(),
                ...
                toggleClass = function () {
                    $('.date-items li').removeClass(SELECT_CLASS);
        ...
        event: function () {
            ...
            _this.$element.on('click', ARROW_DATE, function () {
                var arr = _this.getDisDateValue(),
                    ...
                    y = arr[0],
                    m = arr[1];
                var thisClickTime = getCookie('thisClickTime');
                var Btny = y;
                if (type == 'prev') {
                    var Btnm = m - 1;
                    if (Btnm == 0) {
                ...
                if (thisClickTime) {
                    var thisClickTimeArr = thisClickTime.split("-");
                    var thisClicky = thisClickTimeArr[0];
                    var thisClickm = thisClickTimeArr[1];

                }
                var newData = formatDateTime(new Date());
                var newDataArr = newData.split("-");
                var newDatay = newDataArr[0];
                var newDatam = newDataArr[1];
                var d = _this.updateDateView(y, m, type, function () {
                    ...
                    this.$dateItems.children(':eq(1)')
                        ...
                        .removeClass(SELECT_CLASS)
                    if (thisClickTime) {
                        if (Btny == thisClicky && Btnm == parseInt(thisClickm)) {
                            var time1 = thisClickTimeArr[2]
                            _this.selectedDay(time1, '')
                        }
                    }

                    if (Btny == newDatay && Btnm == newDatam) {
                        this.$dateItems.children(':eq(1)')
                            .find('[' + ITEM_DAY + ']:not(.' + NEW_DAY_CLASS + ', .' + OLD_DAY_CLASS + ')')
                            .removeClass(TODAY_CLASS)
                            .filter(function (index) {
                                return parseInt(this.innerHTML) == newDataArr[2];
            ...
            _this.$element.on('click', '[' + ITEM_DAY + ']', function () {
                ...
                var thisTime = formatDateTime(day)
                setCookie('thisClickTime', thisTime, 7);
...
if (getCookie('thisClickTime')) {
    removeCookie('thisClickTime')
}
